...
// Code template snippet found here: https://codepen.io/eyalcohen4/pen/mprbzP
const renderSpinner = () => {
...
const searchScorers = async (playerSearchText) => {
  ...
  let matches = scorers.filter((scorer) => {
    ...
    // code block adapted from https://stackoverflow.com/questions/990904/remove-accents-diacritics-in-a-string-in-javascript
    return (
      scorer.fname
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
      ...
      scorer.lname
        ...
        .replace(/[\u0300-\u036f]/g, "")
        .match(regex) ||
      scorer.club.match(regex) ||
      scorer.clubAcronym.match(regex)
    );
  });
  ...
  if (playerSearchText.length > 2 && matches.length === 0) {
    playerMatchList.innerHTML =
      "<div class='alert alert-warning text-center' role='alert'><span class='icon icon-warning'></span><br><p>Sorry, no scorers found.</p><p>Please try searching for someone else!</p></div>";
  } else if (playerSearchText.length > 2) {
    showScorers(matches);
  } else if (playerSearchText.length === 0) {
    matches = [];
    playerMatchList.innerHTML = "";
...
const searchClubs = async (clubSearchText) => {
  ...
  if (clubSearchText.length > 0 && matches.length === 0) {
    clubMatchList.innerHTML =
      "<div class='alert alert-warning text-center' role='alert'><span class='icon icon-warning'></span><br><p>Sorry, no clubs found.</p><p>Please try searching for another one!</p></div>";
  } else if (clubSearchText.length > 0) {
    showClubs(matches);
  } else if (clubSearchText.length === 0) {
    matches = [];
    clubMatchList.innerHTML = "";
...
const showScorers = (matches) => {
  if (matches.length > 0) {
    const scorerList = matches
      .map(
        (match) => `<div class="col-12 px-0"><div class="row px-3">
      <div class="statcard col-9 statcard-primary statcard-${match.club
        .replace(/\s/g, "")
        .replace("&", "and")
        .toLowerCase()} pl-2 py-2 pr-2 p-lg-3 mb-1">
        <h2 class="statcard-number">
          ${match.fname} ${match.lname}
        </h2>
        <span class="statcard-desc pt-1 statcard-desc-${match.club
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()}">${match.club}</span></div>
        <div class="statcard col-3 d-flex statcard-primary statcard-secondary-${match.club
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()} pr-2 py-2 mb-1">
        <a href="#" class="btn-strike-rate btn-strike-rate-${match.club
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()} mb-1 btn btn-outline-primary px-1" title="Load scorer dashboards"><span class="icon icon-bar-graph"></span></a>
      </div></div>
    </div>`
      )
      .join("");
...
const showClubs = (matches) => {
  if (matches.length > 0) {
    const clubList = matches
      .map(
        (match) => `<div class="col-12 px-0"><div class="row px-3">
        <div class="statcard col-8 statcard-primary statcard-${match.clubname
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()} pl-2 py-2 pr-2 p-lg-3 mb-1">
          <h2 class="statcard-number statcard-number-clubs" id="${match.APIclubname.split(
            " "
          ).join("")}">
            ${match.clubname}
          </h2>
          <span class="statcard-desc pt-1 statcard-desc-${match.clubname
            .replace(/\s/g, "")
            .replace("&", "and")
            .toLowerCase()}">#${match.acronym}</span></div>
          <div class="statcard col-2 d-flex statcard-crest statcard-crest-${match.clubname
            .replace(/\s/g, "")
            .replace("&", "and")
            .toLowerCase()} pl-1 pr-2 py-2 mb-1">
          <img class="club-crest" src="${
            match.crestURL
          }" height="48px" width="100%">
        </div><div class="statcard col-2 d-flex px-0 mb-1 statcard-line-chart statcard-line-chart-${match.clubname
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()}">
        <a href="#" class="d-flex btn-strike-rate-club btn-strike-rate-club-${match.clubname
          .replace(/\s/g, "")
          .replace("&", "and")
          .toLowerCase()}" title="Load club dashboards"><i class="d-flex fas fa-chart-line"></i></a>
      </div></div>
      </div>`
      )
      .join("");
...
$(document).ready(function () {
  ...
  $(document).on("click", ".btn-strike-rate", function () {
    ...
    function scorerSummary(profile, value, goals, _goals, goals_, gpm, _gpm) {
      return `<div class="col-12 statcard-img border-bottom-0 px-0"><img class="d-block mx-auto mt-4 mb-3" src="${profile.playerImage}" alt="${profile.playerName}" height="auto" width="180"><div class="col-12 mb-4 px-0 border-top-0 statcard-img-name text-center"><h3>${profile.playerName}</h3></div></div><nav class="iconav mt-0 px-0 mb-0 col-12"><div class="iconav-slider row"><div class="col-12"><ul class="row nav nav-pills iconav-nav mx-auto" id="scorers-tab" role="tablist"><li class="nav-item px-0 col-4" role="presentation"><a class="nav-link nav-link-scorers active" id="scorers-players-tab" href="#scorers-players" title="Load summary dashboard" role="tab" aria-controls="scorers-players" aria-selected="true" data-toggle="pill"><span class="icon iconav-icon icon-list"></span><small class="iconav-nav-label">Summary</small></a></li><li class="nav-item ml-0 px-0 col-4" role="presentation"><a class="nav-link nav-link-scorers" id="scorers-strikerate-tab" href="#scorers-strikerate" title="Load StrikeRate dashboard" role="tab" aria-controls="scorers-strikerate" aria-selected="false" data-toggle="pill"><span class="icon iconav-icon icon-circular-graph"></span><small class="iconav-nav-label">StrikeRate</small></a></li><li class="nav-item ml-0 px-0 col-4" role="presentation"><a class="nav-link nav-link-scorers" id="scorers-strikevalue-tab" href="#scorers-strikevalue" title="Load StrikeValue dashboard" role="tab" aria-controls="scorers-strikevalue" aria-selected="false" data-toggle="pill"><span class="icon iconav-icon icon-line-graph"></span><small class="iconav-nav-label">StrikeValue</small></a></li></ul></div></div></nav><div class="col-12 px-0 tab-content statcard-body-wrapper" id="scorers-tab-content"><div class="row statcard-body-row mx-0 tab-pane fade show active" id="scorers-players" role="tabpanel" aria-labelledby="scorers-players-tab"><div class="col-12 border-top-0 py-1 py-lg-3 statcard-body"><div class="row statcard-row statcard p-3 py-md-4"><div class="col-4 statcard-col text-center"><h4 class="statcard-stat statcard-number">${profile.age}</h4><span class="statcard-desc">Age</span></div><div class="col-4 statcard-col"></div><div class="col-4 statcard-col text-center"><h4 class="statcard-stat statcard-number">${profile.playerShirtNumber}</h4><span class="statcard-desc">Number</span></div></div><div class="row statcard-row statcard px-3 py-md-2 py-lg-3"><div class="crest-box col-4 px-0 statcard-col d-flex"><img src="${profile.clubImage}" alt="${profile.club}" width="50%"></div><div class="user-box col-4 py-3 py-md-2 px-0 statcard-col d-flex"><span class="icon icon-user d-flex"></span></div><div class="crest-box col-4 px-0 statcard-col d-flex"><img src="${profile.countryImage}" alt="${profile.country}" width="40%"></div></div><div class="row statcard-row statcard p-3 py-md-4"><div class="col-4 statcard-col text-center"><h4 class="statcard-stat statcard-number">${goals}</h4><span class="statcard-desc">Goals</span></div><div class="col-4 statcard-col"></div><div class="col-4 statcard-col text-center"><h4 class="statcard-stat statcard-number" id="statcard-player-value-stat">€${value}m</h4><span class="statcard-desc">Value</span></div></div></div></div><div class="statcard-body-container tab-pane fade show" id="scorers-strikerate" role="tabpanel" aria-labelledby="scorers-strikerate-tab"><div class="row statcard-body-row mx-0"><button type="button" class='d-block px-md-4 py-md-2 mx-auto mt-3 mt-md-4 mb-2 mb-md-3 btn btn-sm btn-pill btn-warning' id="toggle-pens">Exclude pens</button><div class="col-12 border-top-0 statcard-body statcard-canvas px-4 pb-4 pt-1"><canvas id="myChart"></canvas></div></div></div><div class="statcard-body-container tab-pane fade show" id="scorers-strikevalue" role="tabpanel" aria-labelledby="scorers-strikevalue-tab"><div class="row statcard-body-row mx-0"><button type="button" class='d-block px-md-4 py-md-2  mx-auto mt-3 mt-md-4 mb-2 mb-md-3 btn btn-sm btn-pill btn-secondary' id="toggle-penz">Exclude pens</button><div class="col-12 border-top-0 statcard-body statcard-canvas px-4 pb-4 pt-1"><canvas id="mySecondChart"></canvas></div></div></div></div><script>var ctx = document.getElementById('myChart').getContext('2d');var chart = new Chart(ctx, {type: 'doughnut',data: {datasets: [{data: [${_goals}, 1.08 - ${_goals}],backgroundColor: ['#1bc98e', '#252830'],borderColor: ['#fff', '#252830'],borderAlign: 'inner'}]}, options: {responsive:true, cutoutPercentage: 60, maintainAspectRatio: false, title: {display: true,position: 'bottom',fontSize: 20, fontStyle: 'normal', text: '${_goals} goals per game',fontColor: '#fff'}, legend: {display: false}, tooltips: {callbacks: {label: function(tooltipItem) {return tooltipItem.yLabel;}}}}}); var pensToggler = document.getElementById('toggle-pens'); function resetStrikeRatePane() {pensToggler.innerText = 'Exclude pens';pensToggler.classList.add('btn-warning'); pensToggler.classList.remove('btn-success'); chart.config.data.datasets[0].data = [${_goals}, 1.08 - ${_goals}]; chart.config.data.datasets[0].backgroundColor = ['#1bc98e', '#252830'];chart.config.options.title.text = '${_goals} goals per game';chart.update();} let scorersPlayersTabsArray = document.querySelectorAll(".nav-link-scorers");scorersPlayersTabsArray.forEach(function(tab) {tab.addEventListener("click", resetStrikeRatePane);});pensToggler.addEventListener('click', function() {this.classList.toggle('btn-warning');this.classList.toggle('btn-success');if(this.classList.contains('btn-success')) {this.innerText = 'Include pens';chart.config.data.datasets[0].data = [${goals_}, 1.08 - ${goals_}];chart.config.data.datasets[0].backgroundColor = ['#e4d836', '#252830'];chart.config.options.title.text = '${goals_} goals per game';chart.update();} else {this.innerText = 'Exclude pens';chart.config.data.datasets[0].data = [${_goals}, 1.08 - ${_goals}];chart.config.data.datasets[0].backgroundColor = ['#1bc98e', '#252830'];chart.config.options.title.text = '${_goals} goals per game';chart.update();};});var _ctx = document.getElementById('mySecondChart').getContext('2d'); var _chart = new Chart(_ctx, {type: 'doughnut',data: {datasets: [{data: [${gpm}, 4.0 - ${gpm}],backgroundColor: ['#9f86ff', '#252830'],borderColor: ['#fff', '#252830'],borderAlign: 'inner'}]},options: {responsive:true, cutoutPercentage: 60, maintainAspectRatio: false, title: {display: true,position: 'bottom',fontSize: 20,fontStyle: 'normal',text: '${gpm} goals per €1m',fontColor: '#fff'}, legend: {display: false}, tooltips: {callbacks: {label: function(tooltipItem) {return tooltipItem.yLabel;}}}}});var penzToggler = document.getElementById('toggle-penz');function resetStrikeValuePane() {penzToggler.innerText = 'Exclude pens';penzToggler.classList.add('btn-secondary');penzToggler.classList.remove('btn-info');_chart.config.data.datasets[0].data = [${gpm}, 4.0 - ${gpm}];_chart.config.data.datasets[0].backgroundColor = ['#9f86ff', '#252830'];_chart.config.options.title.text = '${gpm} goals per €1m';_chart.update();}scorersPlayersTabsArray.forEach(function(tab) {tab.addEventListener("click", resetStrikeValuePane);});penzToggler.addEventListener('click', function() {this.classList.toggle('btn-secondary');this.classList.toggle('btn-info');if(this.classList.contains('btn-info')) {this.innerText = 'Include pens';_chart.config.data.datasets[0].data = [${_gpm}, 4.0 - ${_gpm}];_chart.config.data.datasets[0].backgroundColor = ['#f5f5f5', '#252830'];_chart.config.options.title.text = '${_gpm} goals per €1m';_chart.update();} else {this.innerText = 'Exclude pens';_chart.config.data.datasets[0].data = [${gpm}, 4.0 - ${gpm}];_chart.config.data.datasets[0].backgroundColor = ['#9f86ff', '#252830'];_chart.config.options.title.text = '${gpm} goals per €1m';_chart.update();};});</script>`;
    }
    ...
    var getScorerInfo = function () {
      class scorerSettings {
        constructor() {
          ...
          this.headers = {
            "x-rapidapi-key":
              "750dd332b2msh2ea2cd6530f8ce8p182023jsn0aa726d7814f",
      ...
      $.ajax(getScorerID).done(function (response) {
        ...
        if (response.count.players === 0) {
          ...
          setTimeout(() => {
            playerMatchList.innerHTML =
              "<div class='alert alert-danger text-center' role='alert'><span class='icon icon-warning'></span><br><p>We're sorry, but something's gone wrong.</p><p>Please refresh the page and try again!</p></div>";
        ...
        $.when(
          ...
          function (responseValue, responseProfile, responseGoals) {
            ...
            var currentMarketValue = responseValue[0].marketValueDevelopment[0].marketValue
              .replace(",", ".")
              .slice(0, -1);
            var roundedMarketValue;
            if (scorerID === "49542") {
                currentMarketValue = 0.8;
                roundedMarketValue = 0.8;
            } else if (
              currentMarketValue[currentMarketValue.length - 1] !== "0"
            ) {
            ...
            for (let i = 0; i < responseGoals[0].length; i++) {
              if (`${scorerHTMLName}` === `${responseGoals[0][i].fullname}`) {
                ...
                var goalsMinusPensPerMillion =
                  goalsMinusPens / currentMarketValue;
  ...
  $(document).on("click", ".btn-strike-rate-club", function () {
    ...
    function clubSummary(
      ...
      return `<div class="col-12 statcard-img statcard-img-${goals.acronym.toLowerCase()} border-bottom-0 px-0"><img class="d-block mx-auto mt-4 mb-3" src="${goals.crestURL}" alt="${goals.clubname} club crest" height="auto" width="180"><div class="col-12 mb-4 px-0 border-top-0 statcard-img-name text-center"><h3 class="d-none d-sm-block">${goals.clubname}</h2></div></div><nav class="iconav mt-0 px-0 mb-0 col-12"><div class="iconav-slider row"><div class="col-12"><ul class="row nav nav-pills iconav-nav mx-auto" id="clubs-tab" role="tablist"><li class="nav-item px-0 col-4" role="presentation"><a class="nav-link nav-link-clubs active" id="scorers-clubs-tab" href="#scorers-clubs" title="Load club summary dashboard" role="tab" aria-controls="scorers-clubs" aria-selected="true" data-toggle="pill"><span class="icon iconav-icon icon-list"></span><small class="iconav-nav-label">Summary</small></a></li><li class="nav-item ml-0 px-0 col-4" role="presentation"><a class="nav-link nav-link-clubs" id="clubs-strikerate-tab" href="#clubs-strikerate" title="Load club StrikeRate dashboard" role="tab" aria-controls="clubs-strikerate" aria-selected="false" data-toggle="pill"><span class="icon iconav-icon icon-circular-graph"></span><small class="iconav-nav-label">StrikeRate</small></a></li><li class="nav-item ml-0 px-0 col-4" role="presentation"><a class="nav-link nav-link-clubs" id="clubs-strikevalue-tab" href="#clubs-strikevalue" title="Load club StrikeValue dashboard" role="tab" aria-controls="clubs-strikevalue" aria-selected="false" data-toggle="pill"><span class="icon iconav-icon icon-line-graph"></span><small class="iconav-nav-label">StrikeValue</small></a></li></ul></div></div></nav><div class="col-12 px-0 tab-content statcard-body-wrapper" id="clubs-tab-content"><div class="row statcard-body-row mx-0 tab-pane fade show active" id="scorers-clubs" role="tabpanel" aria-labelledby="scorers-clubs-tab"><div class="col-12 border-top-0 statcard-body py-md-3"><div class="row statcard-row statcard py-3 py-md-4 px-2"><div class="col-5 px-0 statcard-col text-center"><h4 class="statcard-stat statcard-number">${age.slice(0, -1)}</h4><span class="statcard-desc pb-md-3 pb-lg-0">Average age</span></div><div class="col-2 statcard-col"></div><div class="col-5 px-0 statcard-col text-center"><h4 class="statcard-stat statcard-number">${position}</h4><span class="statcard-desc pb-md-3 pb-lg-0">League position</span></div></div><div class="row statcard-row statcard py-0 py-md-2 py-lg-3 px-2"><div class="col-4 statcard-col text-center"></div><div class="user-box col-4 py-3 py-md-2 px-0 statcard-col d-flex"><span class="icon icon-sports-club d-flex"></span></div><div class="col-4 statcard-col text-center"></div></div><div class="row statcard-row statcard px-2 py-3 py-md-4"><div class="col-5 px-0 statcard-col text-center"><h4 class="statcard-stat statcard-number">${goals.goals}</h4><span class="statcard-desc">Goals</span></div><div class="col-2 statcard-col"></div><div class="col-5 px-0 statcard-col text-center"><h4 class="statcard-stat statcard-number" id="statcard-value-stat">€${_value}m</h4><span class="statcard-desc">Average player value</span></div></div></div></div><div class="statcard-body-container tab-pane fade show" id="clubs-strikerate" role="tabpanel" aria-labelledby="clubs-strikerate-tab"><div class="row statcard-body-row mx-0"><button type="button" class='d-block px-md-4 py-md-2 mx-auto mt-3 mt-md-4 mb-2 mb-md-3 btn btn-sm btn-pill btn-warning' id="pens-toggle">Exclude pens</button><div class="col-12 border-top-0 statcard-body statcard-canvas px-4 pb-4 pt-1"><canvas id="myClubsChart"></canvas></div></div></div><div class="statcard-body-container tab-pane fade show" id="clubs-strikevalue" role="tabpanel" aria-labelledby="clubs-strikevalue-tab"><div class="row statcard-body-row mx-0"><button type="button" class='d-block px-md-4 py-md-2 mx-auto mt-3 mt-md-4 mb-2 mb-md-3 btn btn-sm btn-pill btn-secondary' id="penz-toggle">Exclude pens</button><div class="col-12 border-top-0 statcard-body statcard-canvas px-4 pb-4 pt-1"><canvas id="mySecondClubsChart"></canvas></div></div></div></div><script>var config = document.getElementById('myClubsChart').getContext('2d');var clubsChart = new Chart(config, {type: 'doughnut',data: {datasets: [{data: [${(goals.goals / 38).toFixed(2)}, 2.18 - ${(goals.goals / 38).toFixed(2)}],backgroundColor: ['#1bc98e', '#252830'],borderColor: ['#fff', '#252830'],borderAlign: 'inner'}]},options: {responsive:true, cutoutPercentage: 60, maintainAspectRatio: false,title: {display: true,position: 'bottom',fontSize: 20,fontStyle: 'normal',text: '${(goals.goals / 38).toFixed(2)} goals per game',fontColor: '#fff'}, legend: {display: false}, tooltips: {callbacks: {label: function(tooltipItem) {return tooltipItem.yLabel;}}}}});var _pensToggler = document.getElementById('pens-toggle');function resetClubStrikeRatePane() {_pensToggler.innerText = 'Exclude pens';_pensToggler.classList.add('btn-warning');_pensToggler.classList.remove('btn-success');clubsChart.config.data.datasets[0].data = [${(goals.goals / 38).toFixed(2)}, 2.18 - ${(goals.goals / 38).toFixed(2)}];clubsChart.config.data.datasets[0].backgroundColor = ['#1bc98e', '#252830'];clubsChart.config.options.title.text = '${(goals.goals / 38).toFixed(2)} goals per game';clubsChart.update();}let scorersClubsTabsArray = document.querySelectorAll(".nav-link-clubs");scorersClubsTabsArray.forEach(function(tab) {tab.addEventListener("click", resetClubStrikeRatePane);});_pensToggler.addEventListener('click', function() {this.classList.toggle('btn-warning');this.classList.toggle('btn-success');if(this.classList.contains('btn-success')) {this.innerText = 'Include pens';clubsChart.config.data.datasets[0].data = [${(goals.goalsMinusPens / 38).toFixed(2)}, 2.05 - ${(goals.goalsMinusPens / 38).toFixed(2)}];clubsChart.config.data.datasets[0].backgroundColor = ['#e4d836', '#252830'];clubsChart.config.options.title.text = '${(goals.goalsMinusPens / 38).toFixed(2)} goals per game';clubsChart.update();} else {this.innerText = 'Exclude pens';clubsChart.config.data.datasets[0].data = [${(goals.goals / 38).toFixed(2)}, 2.18 - ${(goals.goals / 38).toFixed(2)}];clubsChart.config.data.datasets[0].backgroundColor = ['#1bc98e', '#252830'];clubsChart.config.options.title.text = '${(goals.goals / 38).toFixed(2)} goals per game';clubsChart.update();};});var _config = document.getElementById('mySecondClubsChart').getContext('2d');var _clubsChart = new Chart(_config, {type: 'doughnut',data: {datasets: [{data: [${gp100m}, 28.92 - ${gp100m}],backgroundColor: ['#9f86ff', '#252830'],borderColor: ['#fff', '#252830'],borderAlign: 'inner'}]},options: {responsive:true, cutoutPercentage: 60, maintainAspectRatio: false, title: {display: true,position: 'bottom',fontSize: 20,fontStyle: 'normal',text: '${gp100m} goals per €100m',fontColor: '#fff'}, legend: {display: false}, tooltips: {callbacks: {label: function(tooltipItem) {return tooltipItem.yLabel;}}}}});var _penzToggler = document.getElementById('penz-toggle');function resetClubStrikeValuePane() {_penzToggler.innerText = 'Exclude pens';_penzToggler.classList.add('btn-secondary');_penzToggler.classList.remove('btn-info');_clubsChart.config.data.datasets[0].data = [${gp100m}, 28.92 - ${gp100m}];_clubsChart.config.data.datasets[0].backgroundColor = ['#9f86ff', '#252830'];_clubsChart.config.options.title.text = '${gp100m} goals per €100m';_clubsChart.update();}scorersClubsTabsArray.forEach(function(tab) {tab.addEventListener("click", resetClubStrikeValuePane);});_penzToggler.addEventListener('click', function() {this.classList.toggle('btn-secondary');this.classList.toggle('btn-info');if(this.classList.contains('btn-info')) {this.innerText = 'Include pens';_clubsChart.config.data.datasets[0].data = [${gmpp100m}, 27.05 - ${gmpp100m}];_clubsChart.config.data.datasets[0].backgroundColor = ['#f5f5f5', '#252830'];_clubsChart.config.options.title.text = '${gmpp100m} goals per €100m';_clubsChart.update();} else {this.innerText = 'Exclude pens';_clubsChart.config.data.datasets[0].data = [${gp100m}, 28.92 - ${gp100m}];_clubsChart.config.data.datasets[0].backgroundColor = ['#9f86ff', '#252830'];_clubsChart.config.options.title.text = '${gp100m} goals per €100m';_clubsChart.update();};});</script>`;
    }
        ...
        $.when(
          ...
          function (
            ...
            for (let i = 0; i < leagueTable.length; i++) {
              if (
                `${clubNameAPI}` ===
                `${leagueTable[i].clubName.split(" ").join("")}`
              ) {
                clubPosition = i + 1;
                if (clubPosition === 1) {
                  clubPosition += "st";
            ...
            // code block adapted from https://stackoverflow.com/questions/24806772/how-to-skip-over-an-element-in-map
            var values = squad
              .filter(function (squadMember) {
            ...
            if (roundedSquadValue.length > 3) {
              _roundedSquadValue =
                insertBillionDecimal(roundedSquadValue) + "bn";
            } else {
              _roundedSquadValue = roundedSquadValue + "m";
            }

            for (let i = 0; i < responseClubGoals[0].length; i++) {
              if (`${clubHTMLName}` === `${responseClubGoals[0][i].clubname}`) {
                clubGoals = responseClubGoals[0][i];
              }
            }
          ...
          function (
            errorResponseLogo,
            errorResponseClubProfile,
            errorResponseClubPosition,
            errorResponseSquadValue,
            errorResponseClubGoals
          ) {
            setTimeout(() => {
              clubMatchList.innerHTML =
                "<div class='alert alert-danger text-center' role='alert'><span class='icon icon-warning'></span><br><p>We're sorry, but something's gone wrong.</p><p>Please refresh the page and try again!</p></div>";
